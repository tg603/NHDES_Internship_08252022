...
fileLocation = 'C:\\Users\\zachary.t.thoroughgo\\DES\\' + logName
desinationLocation = 'C:\\Users\\zachary.t.thoroughgo\\DES\\Environment\\' + logName
...
def readIni():
    try:
        ...
        global jumpreldiffthreshold, pmreldiffthreshold, jumpathreshold, jumpbthreshold, pmdiffthreshold, timethreshold, rhdiffthreshold, rhjumpdiffthreshold, units, keywords, duplicates
        defaults = {
        ...
        pmreldiffthreshold      = str(items['pmreldiffthreshold'])
        jumpathreshold      = str(items['jumpathreshold'])
        jumpbthreshold      = str(items['jumpbthreshold'])
        pmdiffthreshold     = str(items['pmdiffthreshold'])
        timethreshold       = str(items['timethreshold'])
        rhdiffthreshold     = str(items['rhdiffthreshold'])
        rhjumpdiffthreshold     = str(items['rhjumpdiffthreshold'])
                ...
                for i in arrayKeys:
                    ...
                    try:
                        if(clock != 1 and str(jsonObject4.get(i)) != 'None' and dict_5 == False):
                            ...
                            RD_threshold = float(jumpreldiffthreshold)
                            #RD_threshold = 125
                            #rd_threshold = 50
                            rd_threshold = float(pmreldiffthreshold)
                            print(str(rd_threshold))
                            #A_threshold = 7
                            A_threshold = float(jumpathreshold)
                            B_threshold = float(jumpbthreshold)
                            #B_threshold = 7
                            #Diff_threshold = 10
                            Diff_threshold = float(pmdiffthreshold)
                            #time_threshold = 3
                            time_threshold = float(timethreshold)
                            jump_a_index = arrayKeys.index("PM2.5_JumpA")
                            jump_b_index = arrayKeys.index("PM2.5_JumpB")
                            ...
                            try:
                                ...
                                RHDiff_threshold = rhdiffthreshold
                                RHJumpDiff_threshold = rhjumpdiffthreshold
